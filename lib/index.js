...
DCP.setPresence = function(input) {
	var ACCEPTED = ['online', 'idle', 'dnd', 'invisible']; 
	var payload = Events.SET_PRESENCE(
		
		input.idle_since || null, 
...
function handleWSOpen(opts) {
	var ident = Events.IDENTITY(this.internals.token);
...
function handleUDPMessage(voiceSession, msg, rinfo) {
	...
	send(voiceSession.ws.connection, Events.UDP_DISCOVERY(vDiscIP, Number(vDiscPort), voiceSession.ws.modes[1]));
}
