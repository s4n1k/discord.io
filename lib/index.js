...
Discord.Client = function DiscordClient(options) {
	if (type(options) !== 'object') throw new TypeError("An Object is required to create the discord.io client");
	if (type(options.token) !== 'string') throw new TypeError("The discord.io requires a String token value");
	if (type(options.messageCacheLimit) !== 'number') options.messageCacheLimit = 50;
	...
	applyProperties(this, [
		...
		["_messageCacheLimit", Math.max(0, options.messageCacheLimit)],
	]);
...
DCP.connect = function() {
	if (!this.connected && !this._connecting) return setTimeout(
		...
		this,
		arguments[0],
		void( CONNECT_WHEN = Math.max(CONNECT_WHEN, Date.now()) + 6E3 )
	);
};
 ...
 * @arg {Object} input
 * @arg {String} [input.status] - What you want to appear to others 'online', 'idle', 'dnd', or 'invisible'.
...
DCP.setPresence = function(input) {
	var game, status = (Statuses.indexOf(input.status) > -1 ? input.status : (this.internals.lastStatus || 'online') );

	 if ('game' in input && type(input.game) === 'object') {
		 game = {
		 ...
		 game = (input.game === null ? null : this.internals.lastGame);
	 }

	this.internals.lastStatus = status;
	this.internals.lastGame = game;

	if (!this.bot) this._req('patch', Endpoints.ME, {status: status}, function() {});
	return send(this._ws, Events.SET_PRESENCE( status, game ));
};
 ...
 * @arg {Object} [intput.embed] - A Discord Embed object to include.
...
DCP.sendMessage = function(input, callback) {
	...
	if (input.typing === true) {
		return simulateTyping(
			...
			input.to,
			message,
			( (message.content.length * 0.12) * 1E3 ),
			callback
		);
	}
...
DCP.editMessage = function(input, callback) {
	this._req('patch', Endpoints.MESSAGES(input.channelID, input.messageID), generateMessage(input.message || '', embed), function(err, res) {
...
DCP.editChannelInfo = function(input, callback) {
	...
	try {
		...
		for (var key in input) {
			...
			if (+input[key]) {
				if (key === 'bitrate') {
					payload.birate = Math.min( Math.max( input.bitrate, 8E3), 96E3);
					continue;
				}
				if (key === 'user_limit') {
...
DCP.editWebhook = function(input, callback) {
	var client = this, payload = {}, allowed_values = ['avatar', 'name'];
	this._req('get', Endpoints.WEBHOOKS(input.webhookID), function(err, res) {
		...
		allowed_values.forEach(function(key) {
...
DCP.getAudioContext = function(channelObj, callback) {
	...
	var channelID = channelObj.channelID || channelObj, voiceSession = this._vChannels[channelID], encoder = chooseAudioEncoder(Encoders);
...
function cacheMessage(cache, limit, channelID, message) {
	...
	if (limit === Number.MAX_SAFE_INTEGER) return void(cache[channelID][message.id] = message);
...
function simulateTyping(client, to, message, time, callback) {
	...
	client.simulateTyping(to, function() {
		setTimeout(simulateTyping, Math.min(time, 5E3), client, to, message, time - 5E3, callback);
	});
}
...
function getPrivateChannels(client, PCArray) {
	PCArray.forEach(function(channel) {
		switch(channel.type) {
			case 1:
				client.directMessages[channel.id] = new DMChannel(client._uIDToDM, channel);
				break;
			case 2:
				//Haven't seen yet
				break;
...
function getToken(client, opts) {
	if (!isNode) {
		//Read from localStorage? Sounds like a bad idea, 
...
function getGateway(client, opts, token) {
	...
	APIRequest('get', Endpoints.GATEWAY, function (err, res) {
		if (!err && goodResponse(res)) return startConnection(client, opts, (res.body.url + "/?encoding=json&v=" + GATEWAY_VERSION));
...
function handleWSOpen(opts) {
	var ident = Events.IDENTIFY(this.internals.token);
...
function handleWSMessage(opts, data, flags) {
	...
	switch (message.t) {
		case "READY":
			...
			getPrivateChannels(client, _data.private_channels);
...
function keepUDPAlive(VS) {
	...
	if (VS.keepAlivePackets > 0xFFFFFFFE) {
...
function handlevWSMessage(voiceSession, vMessage, vFlags) {
	...
	switch (vData.op) {
		case 2: //Ready (Actually means you're READY to initiate the UDP connection)
			...
			voiceSession.wsKeepAlive = setInterval(send, vData.d.heartbeat_interval, voiceSession.ws.connection, Events.VOICE_WS_KEEP_ALIVE);
			...
			voiceSession.udpKeepAlive = setInterval(keepUDPAlive, 5E3, voiceSession);
			break;
...
function handleUDPMessage(voiceSession, msg, rinfo) {
	...
	send(voiceSession.ws.connection, Events.UDP_DISCOVERY(
		vDiscIP, 
		Number(vDiscPort), 
		voiceSession.ws.modes[1])//'xsalsa20_poly1305'
	);
}
...
function AudioCB(voiceSession, audioChannels, sysEncoder, maxStreamSize) {
	...
	applyProperties(this, [
		...
		["_mixedDecoder", new Opus.OpusEncoder( 48E3, audioChannels )],
	]);
	createEncoder(this, sysEncoder);
	...
	if (maxStreamSize) {
		voiceSession.ws.connection.on('message', function(data, flags) {
			...
			if (!voiceSession.members[data.d.user_id]) {
				...
				voiceSession.members[data.d.user_id].decoder = new Opus.OpusEncoder( 48E3, 2 );
			}
...
function createEncoder(AudioCB, sysEncoder) {
	AudioCB._encoder = ChildProc.spawn(sysEncoder, [
		'-i', 'pipe:0',
		'-map', '0:a',
		'-acodec', 'libopus',
		'-f', 'data',
	...
	AudioCB._write = enc.stdin.write.bind(enc.stdin);
	AudioCB._read = function() {};
}
...
function chooseAudioEncoder(players) {
	if (!(players = players.slice())[0]) return null;
...
Object.defineProperty(Resource.prototype, "creationTime", {
	get: function() { return (+this.id / 4194304) + 14200704E5; },
...
function DMChannel(translator, data) {
	copyKeys(data, this);
	translator[data.recipients[0].id] = data.id;
}
function User(data) {
	copyKeys(data, this);
...
/* Endpoints */
(function () {
	var API = "https://discordapp.com/api/v6";
	var ME  = API + "/users/@me";
...
(function() {
	Events = {
		...
		SET_PRESENCE: function(status, game) {
			return {
				op: 3,
				d: {
					status: status,
		...
		VOICE_WS_KEEP_ALIVE: { "op": 3, "d": null },

		UDP_DISCOVERY: function(ip, port, mode) {
			return {
				"op":1,
				"d":{
					"protocol":"udp",
		...
		SPEAKING_START: { "op":5, "d":{ "speaking": true, "delay": 0 } },
		SPEAKING_END: { "op":5, "d":{ "speaking": false, "delay":0 } }
	}
})();
